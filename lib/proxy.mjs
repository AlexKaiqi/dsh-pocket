...
export const PWA_ROUTES = Object.freeze({
  manifest: '/dsh-pocket.webmanifest',
  serviceWorker: '/dsh-pocket-sw.js',
  icon180: '/dsh-pocket-icon-180.png',
  icon192: '/dsh-pocket-icon-192.png',
  icon512: '/dsh-pocket-icon-512.png',
  iconMaskable512: '/dsh-pocket-icon-maskable-512.png',
});

const PWA_ICON_FILES = new Map([
  [PWA_ROUTES.icon180, new URL('../client/pwa/icon-180.png', import.meta.url)],
  [PWA_ROUTES.icon192, new URL('../client/pwa/icon-192.png', import.meta.url)],
  [PWA_ROUTES.icon512, new URL('../client/pwa/icon-512.png', import.meta.url)],
  [PWA_ROUTES.iconMaskable512, new URL('../client/pwa/icon-maskable-512.png', import.meta.url)],
]);

const PWA_MANIFEST = JSON.stringify({
  id: '/',
  name: 'DSH Pocket',
  short_name: 'DSH Pocket',
  description: '在手机上安全访问并实时操作电脑中的 DeepSeek Harness。',
  ...
  icons: [
    { src: PWA_ROUTES.icon192, sizes: '192x192', type: 'image/png', purpose: 'any' },
    { src: PWA_ROUTES.icon512, sizes: '512x512', type: 'image/png', purpose: 'any' },
    { src: PWA_ROUTES.iconMaskable512, sizes: '512x512', type: 'image/png', purpose: 'maskable' },
  ],
});

// PWA 只提供安装壳，不做离线数据层。导航与 Pocket 自身资源始终走网络，
// DSH 的 /api、SSE、WebSocket 和语音链路完全不进入 Service Worker。
const PWA_SERVICE_WORKER = `'use strict';
...
self.addEventListener('fetch', function (event) {
  ...
  if (event.request.mode === 'navigate' || url.pathname.indexOf('/dsh-pocket-') === 0) {
    event.respondWith(fetch(event.request));
  }
});
`;

/** 注入安装声明；Service Worker 仅在 HTTPS/localhost 安全上下文注册。 */
export const PWA_HEAD_INJECT = `<link rel="manifest" href="${PWA_ROUTES.manifest}" data-dsh-pocket-pwa="1"><meta name="mobile-web-app-capable" content="yes"><meta name="apple-mobile-web-app-capable" content="yes"><meta name="apple-mobile-web-app-title" content="DSH Pocket"><meta name="apple-mobile-web-app-status-bar-style" content="black-translucent"><link rel="apple-touch-icon" sizes="180x180" href="${PWA_ROUTES.icon180}"><script data-dsh-pocket-pwa-register="1">!function(){try{if(self.isSecureContext&&'serviceWorker'in navigator){addEventListener('load',function(){navigator.serviceWorker.register('${PWA_ROUTES.serviceWorker}',{scope:'/'}).catch(function(){});},{once:true});}}catch(e){}}();</script>`;

/** Pocket 自有 PWA 资源由代理直接提供，不转发给 dsh web。 */
function servePwaAsset(req, res) {
  if (req.method !== 'GET' && req.method !== 'HEAD') return false;
  const pathname = new URL(req.url ?? '/', 'http://dsh-pocket.local').pathname;
  let body;
  let headers;
  if (pathname === PWA_ROUTES.manifest) {
    body = Buffer.from(PWA_MANIFEST, 'utf8');
    headers = {
      'content-type': 'application/manifest+json; charset=utf-8',
      'cache-control': 'public, max-age=3600',
    };
  } else if (pathname === PWA_ROUTES.serviceWorker) {
  ...
  } else if (PWA_ICON_FILES.has(pathname)) {
    body = readFileSync(PWA_ICON_FILES.get(pathname));
    headers = {
...
/** 默认注入到经代理的 HTML 文档里：polyfill + 可安装 PWA 壳。 */
export const DEFAULT_INJECT = RANDOM_UUID_POLYFILL + PWA_HEAD_INJECT;
